...
export async function POST(request: NextRequest) {
  try {
    const { proposalId, recipientEmail, recipientName, customMessage } = await request.json()

    if (!proposalId || !recipientEmail) {
      return NextResponse.json({ error: "Proposal ID and recipient email are required" }, { status: 400 })
    }
    ...
    const templateVars = {
      client_name: recipientName || proposal.clients?.name || "Valued Client",
      sender_name: settings.sender_name,
      proposal_title: proposal.title,
      proposal_date: new Date(proposal.date).toLocaleDateString(),
      total_amount: proposal.total.toFixed(2),
